...
async function ProductPageContent({
  ...
  const [categories, { products, totalPages, total }] = await Promise.all([
    getRootCategories(),
    getPaginatedProducts(currentPage, 20, categorySlug, sort),
  ]);

  const buildPaginationHref = (p: Record<string, string | number>) =>
    buildHrefHelper(categorySlug, sort, view, p);
  const buildFilterHref = (p: Record<string, string>) =>
    buildHrefHelper(categorySlug, sort, view, { ...p, page: "" });
        ...
        <div className="flex justify-center items-center">
          <div className="inline-flex items-center border border-border divide-x divide-border">
            <Link
              href={
                currentPage > 1
                  ? buildPaginationHref({ page: currentPage - 1 })
                  : "#"
              }
    ...
    <main className="min-h-screen bg-background pt-6 pb-16">
      <Container outerStyle="">
        <Suspense fallback={<ProductSkeletonGrid />}>
          <ProductPageContent searchParams={props.searchParams} />
        </Suspense>
      </Container>
    </main>
  );
